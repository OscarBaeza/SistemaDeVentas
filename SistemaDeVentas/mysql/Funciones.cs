...
namespace SistemaDeVentas.mysql
{
    class Funciones
    {
        ...
        public static int AgregarEmpleado(EmpleadosDAO add)
        {
            ...
            //Utilizamos el siguiente comando SQL para agregarlo a la base de datos
            MySqlCommand comando = new MySqlCommand(String.Format("INSERT INTO PRODUCTOS(NOMBRE, DESCRIPCION, " +
                "ALMACEN, PRECIO)values('{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}','{8}')", add.Nombre, add.ApellidoPaterno, add.ApellidoMaterno, add.Email, add.password, add.password, add.Encargado, add.Date, add.Direccion, add.Telefono), Conexion.obtenerConexion());
            retorno = comando.ExecuteNonQuery();
            return retorno;
        }
        ...
        public static List<EmpleadosDAO> mostrarEmpleado()
        {
            ...
            while (reader.Read())
            {
                ...
                c.Encargado = reader.GetBoolean(6);
        ...
        public static EmpleadosDAO BuscarEmpleado(string codigo)
        {
            ...
            while (reader.Read())
            {
                ...
                c.Encargado = reader.GetBoolean(5);
        ...
        public static int EditarEmpleados(int IdEmpleado, String Nombre, String ApellidoPaterno, String ApellidoMaterno, String Email, String password, Boolean Encargado, String Date, String Direccion, String Telefono)
        {
            //Comando SQL que editara los datos que le mandemos al registro de la base de datos
            MySqlCommand comando = new MySqlCommand(String.Format("UPDATE EMPLEADOS SET NOMBRE='{0}', APELLIDOPATERNO='{1}', APELLIDOMATERNO='{2}', EMAIL='{3}',PASSWORD='{4}',ENCARGADO='{5}',FECHANACIMIENTO='{6}',DIRECCION='{7}',TELEFONO='{8}', WHERE IDPRODUCTO='{9}';", Nombre, ApellidoPaterno, ApellidoMaterno, Email, password, Encargado, Date, Direccion, Telefono, IdEmpleado), Conexion.obtenerConexion());
            //La variable editado será retornada y nos ayudará a saber si se actualizarón los datos de la base de datos
            int editado = comando.ExecuteNonQuery();
            return editado;
        }
